 ...
 * @final
 */
class ImportMappingDoctrineCommand extends DoctrineImportMappingDoctrineCommand
{
    /** @var string[] */
    private $bundles;
    ...
    {
        parent::__construct($doctrine, $bundles);
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('class_to_extend', null, InputOption::VALUE_OPTIONAL, 'Fields to exclude in the mapping information')
            ->addOption('excludefields', null, InputOption::VALUE_OPTIONAL | InputOption::VALUE_IS_ARRAY, 'Class to extend entity from')
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (isset($this->bundles[$namespaceOrBundle])) {
            ...
            if (in_array($type, ['annotation','annotation-extended'])) {
                $destPath .= '/Entity';
            } else {
        ...
        $classToExtend = $input->getOption('class_to_extend');
        $excludefields = $input->getOption('excludefields');
        ...
        if ($metadata) {
            ...
            foreach ($metadata as $class) {
                ...
                if ('annotation-extended' === $type) {
                    $exporter->setClassToExtend($classToExtend);
                }
                $code = $exporter->exportClassMetadata($class, $classToExtend, $excludefields);
                $dir  = dirname($path);
                if (! is_dir($dir)) {
                    mkdir($dir, 0775, true);
                }
