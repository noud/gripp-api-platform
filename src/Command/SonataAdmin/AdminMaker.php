...
namespace App\Command\SonataAdmin;
 ...
 */
final class AdminMaker extends AbstractMaker implements MakerInterface
{
    /**
     * @var string
     */
    ...
    private function fieldsToString($fields, string $adminClassName = '', string $for = '')
    {
        ...
        foreach ($fields as $field) {
            ...
            switch ($for) {
                case 'list':
                    if ('color' === $field) {
                        $fieldString = $fieldString.sprintf('%12s', '')."->add('".$field."', null, [
                ...
                case 'form':
                    ...
                    if (!('Timelineentry' === $adminClassName) && in_array($field, $dateFields)) {
                        $fieldString = $fieldString.sprintf('%12s', '')."->add('".$field."', \Sonata\Form\Type\DatePickerType::class)".PHP_EOL;
                        break;
                    }
                    if ('Timelineentry' === $adminClassName && in_array($field, $dateTimeFields)) {
                        $fieldString = $fieldString.sprintf('%12s', '')."->add('".$field."', \Sonata\Form\Type\DateTimePickerType::class)".PHP_EOL;
                        break;
                    }
                    if (in_array($field, $timeFields)) {
                        $fieldString = $fieldString.sprintf('%12s', '')."->add('".$field."', \Sonata\Form\Type\DateTimePickerType::class, [
                'format' => 'kk:mm',
    ...
    private function generateAdmin(
        ...
        $adminClassFullName = $adminClassNameDetails->getFullName();
        $adminClassShortName = str_replace('Admin', '', $adminClassNameDetails->getShortName());

        $fields = $this->modelManager->getExportFields($this->modelClass);
        $fieldString = $this->fieldsToString($fields);
        $searchFieldString = $this->fieldsToString($fields, $adminClassShortName, 'search');
        $listFieldString = $this->fieldsToString($fields, $adminClassShortName, 'list');
        $formFieldString = $this->fieldsToString($fields, $adminClassShortName, 'form');
        ...
        $generator->generateClass(
            ...
            [
                'fields' => $fieldString,
                'searchFields' => $searchFieldString,
                'listFields' => $listFieldString,
                'formFields' => $formFieldString,
            ]
        );
